...
try {
    ...
    & $Uv.Source run --frozen --extra build --no-default-groups python -c "import PyInstaller"
    if ($LASTEXITCODE -ne 0) {
        throw "PyInstaller is missing from the locked build environment."
    }

    & $Uv.Source run --frozen --extra build --no-default-groups python -m PyInstaller `
        --clean --noconfirm "packaging\windows\LiveTranslator.spec"
    if ($LASTEXITCODE -ne 0) {
    ...
    & $Uv.Source run --frozen --extra build --no-default-groups python `
        (Join-Path $Root "scripts\validate_sbom.py") $DistSbom
    if ($LASTEXITCODE -ne 0) {
